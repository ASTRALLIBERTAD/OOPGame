...
use crate::entity::{Entity, NeutralBehavior};
...
impl ICharacterBody2D for Pet {
    ...
    fn physics_process(&mut self, _delta: f64) {
        ...
        let a_pressed = input.is_physical_key_pressed(Key::A);
        let d_pressed = input.is_physical_key_pressed(Key::D);
        ...
        let pet_position = self.base().get_global_position();
        ...
        if self.is_following {
            if distance > self.stop_threshold {
                ...
                self.sprite.play_ex().name("idle").done();
...
impl Pet {
    fn update_health(&mut self, change: i32) {
        self.current_health = (self.current_health + change).clamp(0, self.max_health);

        godot_print!("Cat HP: {} / {}", self.current_health, self.max_health);
    ...
    fn move_toward_player(&mut self) {
        let direction =
            (self.player.get_global_position() - self.base().get_global_position()).normalized();
    ...
    fn flip_sprite(&mut self) {
        if self.player.get_global_position().x < self.base().get_global_position().x {
...
impl NeutralBehavior for Pet {
    fn interact(&self) -> &'static str {
        "The cat purrs and nuzzles against you."
    }

    fn become_hostile(&mut self) {
        self.is_hostile = true;
        godot_print!("The cat has become hostile!");
    }
}
